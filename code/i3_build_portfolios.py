...
# Extract necessary parameters directly from settings
config_params = {
    ...
    "update_mp": False,  # Model update options can be manually adjusted
    "update_base": False,
...
# Assign values from config_params to settings
...
get_from_path_model = r"C:\Master\ml-implementable-efficient-frontier\code\Outputs"

# Create unique output folder
timestamp = datetime.now().strftime("%Y%m%d-%H%M")
output_path = f"Data/Generated/Portfolios/{timestamp}_WEALTH{pf_set['wealth']}_GAMMA{pf_set['gamma_rel']}_SIZE{config_params['size_screen']}_IND{config_params['industry_cov']}"
os.makedirs(output_path, exist_ok=True)

# Save script files for reproducibility
...
if 'eom' in chars.columns:
    chars['eom'] = pd.to_datetime(chars['eom'], errors='coerce')  # Convert to datetime format
...
daily_returns = load_daily_returns_pkl(settings["data_path"], chars, risk_free)
...
portfolio_data = run_prepare_portfolio_data(
    ...
    settings=settings,
    pf_set=pf_set,
    barra_cov=cov_results["factor_cov"]  # Factor covariance matrix
)
